...
**指针/引用的顶层 `const` 和底层 `const`**

所谓“顶层 `const`”，是指指针/引用本身不可以修改（指向其他数据对象）；所谓“底层 `const`”，是指不可以通过指针/引用修改所指向的数据对象。

**作用**

- 修饰变量，说明该变量不可以被改变，因此 `const` 变量必须有一个初值
  - 修饰指针，分为指向常量的指针（pointer to const）和自身是常量的指针（常量指针，const pointer）；
  - 修饰引用，指向常量的引用（reference to const），用于形参类型，即避免了拷贝，又避免了函数对值的修改,没有 const reference，因为引用只是对象的别名，引用不是对象，不能用 const 修饰
- 修饰成员函数，说明该成员函数内不能修改成员变量。

> **`const_cast`**
> 
> `const_cast` 可以用于移除变量的 cv 限定符（即，const 和 volatile）
...
struct A {
#if !defined(CONST_ONLY)
  void get() { std::cout << "non-const" << std::endl; }
#endif
#if !defined(NON_CONST_ONLY)
  void get() const { std::cout << "const" << std::endl; }
#endif
};
...
void _info(const char *filename, const int lno, const char *msg);

#define macro_info(msg) \
  do { \
    _info(__FILE__, __LINE__, msg); \
  } while (0)

void func_info(const char *msg) {
  _info(__FILE__, __LINE__, msg);
}
...
https://stackoverflow.com/a/583079

`new` 总是将大小作为它的第一个参数。它接收需要分配的数据的字节大小（在本例中为 `sizeof(int)`）。传递给 `new` 的任何额外内容都在辅助参数中传递。
...
如果仅仅将 dtor 设置为 `protected`，那么通过创建一个空 derived class 就可以在栈上分配：
...
C++11 中提供了 [`final`](https://en.cppreference.com/w/cpp/language/final) 关键字，但是 `protected` + `final` 显然不如 `private` 来的方便。

**只能在栈上**

方法：将 new 和 delete 重载为私有

原因：在堆上生成对象，使用 new 关键词操作，其过程分为两阶段：第一阶段，使用 new 在堆上寻找可用内存，分配给对象；第二阶段，调用构造函数生成对象。将 new 操作设置为私有，那么第一阶段就无法完成，就不能够在堆上生成对象（注意到，尽管 placement new 不能重载，但是一旦显式写出了重载的 operator new，placement new 就不能调用）。
